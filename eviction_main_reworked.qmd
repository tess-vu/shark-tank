...
weekly_trend_plot <- ggplot(weekly_aggregate, aes(x = date, y = total_filings)) +
  ...
  geom_vline(xintercept = as.Date("2020-03-01"), 
             linetype = "dashed", color = "#27AE60", alpha = 0.7) + #green line still applies to moratorium beginning, can add labels it helpful
  geom_vline(xintercept = as.Date("2021-09-30"), 
             linetype = "dashed", color = "#E67E22", alpha = 0.7) + #refers to when covid moratorium ends, can add labels if helpful 
  scale_y_continuous(labels = comma) +
  labs(
...
tax_map <- ggplot(philly_tax_sf) +
  ...
  scale_fill_viridis_c(
    ...
    na.value = "gray50",
    breaks = c(15000, 30000, 60000, 90000)
  ) +
  labs(
...
Had to cap the extremes due to it making the model stretch like crazy, obviously this is a huge limitation because this model will not be able to predict mass evictions.
...
```{r model-1}
library(glmmTMB)
# Fit baseline Negative Binomial model with core time, policy, and monthly predictors. I used MASS::glm.nb() instead of glmmTMB because my model kept crashing this is good for big datasets, zero-heavy count distribution. I also changed the beginning of the rest of the code
model_nb_1 <- glmmTMB(
  filings_count ~ filings_ma3 + moratorium_active + factor(month_num),
  data = df_train,
  family = nbinom2(), # this line is added so it is negative binomial
)
...
```{r model-2}
model_nb_2 <- glmmTMB(filings_count ~
...
```{r model-3}
model_nb_3 <- glmmTMB(filings_count ~
                       # 1st model.
                       filings_ma3 + moratorium_active + factor(month_num) +
                       # 2nd model.
                       spatial_lag_filings + delinquent_prop_count +
...
model_nb_4 <- glmmTMB(filings_count ~ # changed from glm.nb to glmmTMB
    # 1st model.
    filings_ma3 + moratorium_active + factor(month_num) +
    # 2nd model.
    spatial_lag_filings + delinquent_prop_count +
  ...
  data = df_train,
  family = nbinom2() # added this line so it is a negative binomial when we make the switch at the beginning 
)
...
```{r model-comparison}
# Compare model fit statistics. Needed to add the following section because glmmTMB does not store $theta so need to get it
get_theta <- function(model) {
  suppressWarnings(as.numeric(sigma(model)))   # returns NA if not NB
}

model_comparison <- data.frame(
  model = c("Baseline", "+ Tax and Spatial", "+ ACS", "+ Interactions"),
  aic = c(AIC(model_nb_1), AIC(model_nb_2), AIC(model_nb_3), AIC(model_nb_4)),
  theta = c(get_theta(model_nb_1), get_theta(model_nb_2), get_theta(model_nb_3), get_theta(model_nb_4)),
  loglik = c(
    as.numeric(logLik(model_nb_1)),
    as.numeric(logLik(model_nb_2)),
    as.numeric(logLik(model_nb_3)),
    as.numeric(logLik(model_nb_4))
  )
) %>%
  mutate(
...
# Extract standardized coefficients for interpretation - Rewritten
library(broom.mixed)
library(dplyr)
library(stringr)

feature_importance <- broom.mixed::tidy(model_nb_enhanced, effect = "fixed") %>%
  filter(term != "(Intercept)") %>%                       # Drop intercept
  mutate(
    abs_estimate = abs(estimate),                        # Absolute value for sorting
    direction = ifelse(estimate > 0, "Positive", "Negative"),
    
    # Clean term names for readability
    term_clean = term %>%
      str_replace("factor\\(month_num\\)", "Month ") %>%
      ...
      str_replace("moratorium_active", "Policy: Moratorium"),
    ...
    category = case_when(
      ...
      str_detect(term_clean, "Moratorium|Policy") ~ "Policy",
      TRUE ~ "Demographic"
    )
  ) %>%
  arrange(desc(abs_estimate)) %>%   # Sort by importance
  slice_head(n = 20)                # Take top 20 features

# View table
feature_importance

# OLD Extract standardized coefficients for interpretation.
