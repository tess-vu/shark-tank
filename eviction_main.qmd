...
summary(df_monthly %>% dplyr::select(filings_count, date, year, moratorium_active))
```
...
summary(df_claims %>% dplyr::select(median_claim, median_claim_baseline, claim_ratio))
```
...
racial_comparison <- racial_stats %>%
  dplyr::select(racial_majority, pct_tracts, pct_total_filings) %>%
...
top_risk_tracts %>%
  dplyr::select(GEOID, racial_majority, total_filings, mean_monthly, cv) %>%
...
# Adjust so that the extreme outliers are not included. Cap at 20 filings, which is 99.8 percentile.
...
### Prediction Calibration by Race
